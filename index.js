...
function createSymbolWithDescription(property) {
  const symKey = Symbol(property); // Створюємо символ symbol з описом property
  return symKey.description; //Повертаємо опис символа symbol
}
...
function createAndCompareTwoSymbols(value) {
  ...
  const b = Symbol(value); // Створюємо два символи з однаковим описом value, та повертаємо результат порівняння двох символів
  return a === b;
}
...
function createSymbolForKey(key) {
  const a = Symbol.for(key); // Використовуємо Symbol.for() для створення нового символу symbol зі значенням key
  ...
  return a; // Повертаємо створений символ
}
...
function useSymbolToStringTag() {
  const myObject1 = {}; // Створюємо пустий об'єкт myObject.
  myObject1[Symbol] = "CustomObject"; // Встановлюємо властивість Symbol об'єкту myObject на значення "CustomObject".
  return myObject1[Symbol].toString(); // Повертаємо рядкове представлення myObject[Symbol].
}
